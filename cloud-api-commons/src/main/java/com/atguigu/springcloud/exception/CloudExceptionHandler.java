...
import com.atguigu.springcloud.domain.vo.Result;
...
//	}
    ...
    public Result<?> handleMaxUploadSizeExceededException(MaxUploadSizeExceededException e) {
		log.error("文件大小超出10MB限制, 请压缩或降低文件质量! " + e.getMessage(), e);
        return Result.error("文件大小超出10MB限制, 请压缩或降低文件质量! ");
    }

    @ExceptionHandler(DataIntegrityViolationException.class)
    public Result<?> handleDataIntegrityViolationException(DataIntegrityViolationException e) {
		log.error("字段太长,超出数据库字段的长度" + e.getMessage(), e);
        return Result.error("字段太长,超出数据库字段的长度");
    }

    @ExceptionHandler({PoolException.class, RedisConnectionException.class})
