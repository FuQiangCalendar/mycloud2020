...
* import org.springframework.data.annotation.Version;*表示任意格式，'text/*’表示任意文本，image/png表示PNG格式的图片；
...
public class HttpProgram {
    ...
    /**
    * @Description : Java标准库提供了基于HTTP的包，但是要注意，早期的JDK版本是通过HttpURLConnection访问HTTP，典型代码如下：
...
//    static HttpClient httpClient = HttpClient.newBuilder().build();
    ...
    /**
    * @Description : 我们来看一下如何使用新版的HttpClient。首先需要创建一个全局HttpClient实例，因为HttpClient内部使用线程池优化多个HTTP连接，可以复用：
     * static HttpClient httpClient = HttpClient.newBuilder().build();
     * 使用GET请求获取文本内容代码如下：
